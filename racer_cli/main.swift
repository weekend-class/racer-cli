...
let winners : [PlayerModel] = players.filter{ player in player.position == cols - 1}
var iconWinner : String = ""

if !winners.isEmpty {
    for index in 0...winners.count - 1 {
        let item : String = index == 0 ? winners[index].icon : ", \(winners[index].icon)"

        iconWinner.append(item)
    }
}
